...
@Suite struct NeedsStateTests {
    ...
    @Test func hungerNeverDropsBelowItsOwnHigherFloor() {
        ...
        needs.decay(over: 1_000 * 3600, traits: neutralTraits, isSleeping: false, isActive: false)
    ...
    @Test func sleepingRestoresEnergyInsteadOfDraining() {
        ...
        needs.decay(over: 3600, traits: neutralTraits, isSleeping: true, isActive: false)
    ...
    @Test func beingActiveFillsPlayInsteadOfDrainingIt() {
        ...
        needs.decay(over: 3600, traits: neutralTraits, isSleeping: false, isActive: true)
        #expect(needs.play > 0.2)
    }

    @Test func idlePlayDecaysInsteadOfRising() {
        var needs = NeedsState.full
        needs.decay(over: 3600, traits: neutralTraits, isSleeping: false, isActive: false)
        #expect(needs.play < 1)
    }
    ...
    @Test func higherLazinessTraitDrainsEnergyFasterWhileAwake() {
        ...
        lazyNeeds.decay(over: 3600, traits: lazy, isSleeping: false, isActive: false)
        energeticNeeds.decay(over: 3600, traits: energetic, isSleeping: false, isActive: false)
