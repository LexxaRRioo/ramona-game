...
- [ ] Grooming (lying variant): row 13, "Lick paw lie front" - same wash motion as the main `groom`, but lying down. Add as a variant (with the same loopPause treatment) for when she's already lying/resting rather than sitting.
...
- [ ] Dedicated drag/held sprite - a distinct alert/prowl pose (slightly different tint, ears back) shown for this, instead of freezing on whatever frame she was on when picked up.
- [ ] Directional sit/stand statics (rows 0/1: front/rear/left/right, unused) - could give her a proper facing sit when she stops mid-turn instead of always the front-facing sitDown/sitIdle pair.
...
- [ ] Lie-down/curl-into-sleep transition should pick one side (left-curl or right-curl) at random each time and stick to frames from only that side for the whole transition, not mix frames from both sides of row 6's sequence.
